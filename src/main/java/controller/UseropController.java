...
import service.impl.UserCoinService;
import service.impl.UserCollectionService;
import service.impl.UserLikesService;
...
public class UseropController {
	...
	private UserLikesService likesService;
	private UserCoinService coinService;

	public UserCoinService getCoinService() {
		return coinService;
	}
	@Resource
	public void setCoinService(UserCoinService coinService) {
		this.coinService = coinService;
	}

	public UserLikesService getLikesService() {
		return likesService;
	}
	@Resource
	public void setLikesService(UserLikesService likesService) {
	...
	public String uploadvideo(HttpServletRequest request,
			...
			// 1、创建一个DiskFileItemFactory工厂
				...
				File linshi = new File("D:\\temp");// 当超过500K的时候，存到一个临时文件夹中
	...
	@RequestMapping(value="/customer/getlikes",method=RequestMethod.GET) /*获取点赞*/
	public @ResponseBody List<Userlikes> getlikes(String userid) {
		return likesService.findAllUserlikes(userid);
	}
	@RequestMapping(value="/customer/cancellikes",method=RequestMethod.GET) /*取消点赞*/
	public @ResponseBody String cancellikes(String userid,String videoid) {
		Video video=userService.getVideo(Integer.parseInt(videoid));
		video.setLikes(video.getLikes()-1);
		userService.updateLikes(video);
		if(likesService.deleteUserlikes(userid,videoid))
			return "success";
		else
			return "fail";
	}
	@RequestMapping(value="/customer/videolikes",method=RequestMethod.GET) /*点赞*/
	public @ResponseBody String videolikes(String userid,String videoid) {
		Video video=userService.getVideo(Integer.parseInt(videoid));
		video.setLikes(video.getLikes()+1);
		userService.updateLikes(video);
		if(likesService.addUserlikes(new Userlikes(userid,videoid)))
			return "success";
		else
			return "fail";
	}
	@RequestMapping(value="/customer/getcoin",method=RequestMethod.GET) /*获取投币*/
	public @ResponseBody List<Usercoin> getcoin(String userid) {
		return coinService.findAllUsercoin(userid);
	}
	@RequestMapping(value="/customer/cancelcoin",method=RequestMethod.GET) /*取消投币*/
	public @ResponseBody String cancelcoin(String userid,String videoid) {
		Video video=userService.getVideo(Integer.parseInt(videoid));
		video.setCoin(video.getCoin()-1);
		userService.updateCoin(video);
		if(coinService.deleteUsercoin(userid,videoid))
			return "success";
		else
			return "fail";
	}
	@RequestMapping(value="/customer/videocoin",method=RequestMethod.GET) /*投币*/
	public @ResponseBody String videocoin(String userid,String videoid) {
		Video video=userService.getVideo(Integer.parseInt(videoid));
		video.setCoin(video.getCoin()+1);
		userService.updateCoin(video);
		if(coinService.addUsercoin(new Usercoin(userid,videoid)))
			return "success";
		else
			return "fail";
	}
